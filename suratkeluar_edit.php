...
if (isset($_GET['id'])) {
    if ($_GET['id'] != "") {
        ...
        $query = mysqli_query($mysqli, "SELECT * FROM suratkeluar WHERE id='$id'");
        $row = mysqli_fetch_array($query);
    } else {
        header("location:halaman_admin_suratkeluar.php");
    }
}
...
if (isset($_POST['id'])) {
    if ($_POST['id'] != "") {
        ...
        $file_nama = $_FILES['file_suratkeluar']['name'];
        $file_size = $_FILES['file_suratkeluar']['size'];
    } else {
        header("location:halaman_admin_suratkeluar.php");
    }
    ...
    if ($file_size > 2097152) {
        ...
        header("location:halaman_admin_suratkeluar.php?pesan=size");
    } else {
        ...
        if ($file_nama != "") {
            ...
            if (in_array($ekstensi, $ekstensi_izin) === true) {
                ...
                $get_file = "SELECT file_suratkeluar FROM suratkeluar WHERE id='$id'";
                $data_file = mysqli_query($koneksi, $get_file);
                // Mengubah data yang diambil menjadi Array
                $file_lama = mysqli_fetch_array($data_file);

                // Menghapus Foto lama didalam folder FOTO
                unlink("pdfsuratkeluar/" . $file_lama['file_suratkeluar']);

                // Memindahkan File kedalam Folder "FOTO"
                move_uploaded_file($file_tmp, 'foto/' . $file_nama_new);

                // Query untuk memasukan data kedalam table SISWA
                $query = mysqli_query($mysqli, "UPDATE suratkeluar SET perihal='$perihal', tanggal_surat='$tanggal_surat', nomor_surat='$nomor_surat', perihal='$perihal', file_suratkeluar='$file_nama_new' WHERE id_siswa='$id'");

                // Mengecek apakah data gagal diinput atau tidak
                if ($query) {
                    header("location:halaman_admin_suratkeluar.php?pesan=berhasil");
                } else {
                    header("location:halaman_admin_suratkeluar.php?pesan=gagal");
                }
            } else {
                // Jika ekstensinya tidak sesuai dengan apa yg kita tetapkan maka error
                header("location:halaman_admin_suratkeluar.php?pesan=ekstensi");
            }
        } else {
...
<body>
    ...
    <div class="container">
        <form name="update_user" method="post" action="halaman_admin_suratkeluar_edit.php" enctype="multipart/form-data">
            <div class="row">
                <div class="col-25">
